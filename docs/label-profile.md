...
- 등급 `C`(비밀)는 N2SF 최상위 등급 — 본문 암호화 대상이며 게이트 내용검사에서 제외된다.
...
## 5. 부록 — 라벨 트레일러 내장 (Phase 1.5, 포맷 불문)

사이드카 유실을 피하고 싶을 때, 포맷별 정식 내장(Phase 2) 전까지 쓰는
공통 내장 방식이다. **원본 바이트는 변경하지 않고 뒤에 덧붙인다.**

```
[원본 바이트][CMS DER][DER 길이: uint64 big-endian 8바이트][매직: "LMLABEL1" ASCII 8바이트]
```

- `contentHash`(서명 대상)는 항상 **원본 바이트** 기준이다. 검증자는 파일 끝
  8바이트가 매직과 일치하면 트레일러를 떼어내고 앞부분만 해시한다.
- 트레일러는 중첩 부착하지 않는다. 재발급은 트레일러를 제거한 원본에 한다.
