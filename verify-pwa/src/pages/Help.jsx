...
const SECTIONS = [
  ...
  { id: 'federation', title: '6. 기관 간 연계 · 원장 화면' }
]
...
// 기관 간 연계(모델 A)와 원장 시각화 안내 — 화면에서 무엇을 누르면 무엇이 기록되는지.
function Federation() {
  return (
    <div>
      <h2>기관 간 연계 · 원장 화면</h2>
      <p><b>현재 기관(페르소나)</b> — 화면 오른쪽 위 "기관" 선택은 <b>내가 지금 어느 기관의 담당자인가</b>입니다.
        발급 화면의 기본 발급기관이 되고, 검증 화면에서는 이 기관의 게이트 입장에서 검증합니다.
        이 데모는 한 서버가 다섯 기관의 서명 키를 모두 갖고 있어 기관을 바꿔 가며 시연할 수 있습니다.
        실제 운영에서는 기관마다 서버와 키가 분리됩니다.</p>
      <p><b>타 기관 문서 검증(협정)</b> — 발급 기관과 검증 기관이 다르면 두 기관 사이의 <b>등가성 협정</b>으로 등급을 번역합니다.
        협정이 없으면 서명의 진위까지만 확인하고 등급은 번역하지 않습니다(검토 권고).
        데모 협정: 우정사업본부(KPOST)↔이노티움(INNOTIUM), 우정사업본부↔행정안전부(MOIS), 우정사업본부↔과학기술정보통신부(MSIT, 2026-12-31 만료).
        국세청(NTS)은 협정이 없는 기관의 예입니다.
        C(비밀) 등급과 잠정(PROVISIONAL) 라벨은 기관 내부 전용이라 항상 차단 권고입니다.
        협정이 만료되면 만료로 표시됩니다.</p>
      <p><b>타 기관으로 보내기 · 수신함</b> — 검증 결과 카드 아래 "타 기관으로 보내기"는 파일을 옮기지 않습니다.
        게이트 관측 로그에 "보냄"만 기록합니다. 기관 선택을 받는 기관으로 바꾸면 검증 화면 <b>수신함</b>에 그 문서가 나타나고,
        "게이트 검증"을 누르면 원장 조회와 협정 번역을 거친 결과가 "검증됨"으로 기록됩니다.
        이 관측 로그는 원장과 별개의 장부입니다. 원장은 발급 기관의 사실, 관측 로그는 게이트가 보고한 사실입니다.</p>
      <p><b>원장 화면 6개 탭</b></p>
      <ul>
        <li><b>표</b> — 원장 행 그대로. 지문 재색인 도구 포함.</li>
        <li><b>레인 차트</b> — 가로가 원장 순번(seq), 세로가 문서. 색이 등급, 색이 바뀌는 곳이 등급변경, 끊기는 곳이 폐기·파기, 세로 점선이 봉인.</li>
        <li><b>전체 그래프</b> — 기관 상자 안의 문서와 파생 관계, 아래쪽 해시체인 척추와 봉인.</li>
        <li><b>분류·온톨로지</b> — 업무분류(BRM) 트리맵, 정보공개법 9조 근거 분포, 태그 관계, 원장 스키마 다이어그램.</li>
        <li><b>기관 지도</b> — 기관 사이의 협정과 등급 번역표, 유효·만료 임박·만료·협정 없음.</li>
        <li><b>기관 간 흐름</b> — 문서 한 건의 등급 띠 위에 찍힌 타 기관 게이트 스탬프, 그리고 발급기관에서 검증기관으로의 흐름.</li>
      </ul>
      <p><b>샘플 파일 로딩(기능테스트)</b> — 원장 화면의 이 버튼은 서버에 준비된 샘플 폴더의 파일들을 여러 기관 이름으로 일괄 발급하고,
        파생·등급변경·폐기·파기·봉인과 기관 간 이동 기록까지 만들어 위 화면들을 채웁니다. 여러 번 눌러도 중복되지 않습니다.</p>
    </div>
  )
}
